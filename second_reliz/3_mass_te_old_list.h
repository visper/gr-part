...
void mass_ini(struct mass* vmass)
{
	struct list* t = 0;
	ALLOC_PTR(t);
	assert(t > 0);
...
int invariant(struct mass* vmass)
{
...
struct node* mass_find(struct mass* vmass, struct data* vdata)
{
	int rc = invariant(vmass);
	...
	if (rc == EMPTY) {
		...
		struct node* temp = vmass->str->head;
		while (temp) {
...
{
	struct node* temp = 0;
	temp = mass_find(vmass, after);
	assert(temp != (struct node*)ERROR);

	//need to check
	//maybe this expression change
	if (temp == (struct node*)ERROR)
		return ERROR;

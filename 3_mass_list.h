...
int three_mass_ins(struct three_mass* vmass, struct vertex* given_row,
		   ...
		   struct vertex_list_node* node_row,
		   struct vertex_list_node* node_column,
		   struct vertex_list_node* node_value)
{
	assert(node_value == node_column == node_row == NULL ||
	       after_value == after_column == after_row == NULL);

	assert(given_row != NULL && given_column != NULL && 
	       given_value != NULL);
...
{
	assert(node_value == node_column == node_row == NULL ||
	       vertex_value == vertex_column == vertex_row == NULL);

	vertex_list_rm(vmass->row, vertex_row, node_row);
	vertex_list_rm(vmass->column, vertex_column, node_column);
	vertex_list_rm(vmass->value, vertex_value, node_value);
